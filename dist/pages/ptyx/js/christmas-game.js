$(function() {
	...
	$(".game_layer_overtime_inner_again").on("touchstart", function() {
		accountValPlayProcess();		
	}); 


	$(".startGameTipButtonImgUrl").on("touchstart", function(e) {
		if($(".game_btn_start_span").html() == "开始" || $(".game_btn_start_span").html() == "继续") {
			$(".game_btn_start_span").html("提示");
			$(".startGameTipButtonImgUrl").css({'background-image':`url(images/${skinType}/main-tipBtn.png)`});
			$(".pic").css("opacity","1");
			$(".game_picture_cover").css("display", "none");
		} else {
			$(".game_btn_start_span").html("继续");
			$(".startGameTipButtonImgUrl").css({'background-image':`url(images/${skinType}/main-continueBtn.png)`});
	...
	var accountValPlayProcess = function(){
		...
		if(!bol){
			if($(".game_btn_start_span").html() == "开始" || $(".game_btn_start_span").html() == "继续") {
				$(".game_btn_start_span").html("提示");
				$(".startGameStartButtonImgUrl").hide();
				$(".game_time").show();
				$(".pic").css("opacity","1");
				$(".game_picture_cover").css("display", "none");
			} else {
				$(".game_btn_start_span").html("继续");
				$(".startGameStartButtonImgUrl").show();
				$(".game_time").hide();
				$(".game_picture_cover").css("display", "block");
				$(".pic").css("opacity","0");		
			}
		...
		opType = 1;
		var url = baseUrl + "puzzle-game-play-costval";
		$.ajax({
			type : "POST",
			url : url,
			data : `activityId=${activityId}&memberId=${memberId}`,
			dataType : "json",
			success : function(msg) {		
				// debugger
				if (msg.result.res == "ok") {
					var gamePlayCost = msg.result.gamePlayCost;
					$("span[name='accountval']").text(gamePlayCost.accountValue);
					if(gamePlayCost.accountValue == 0 && $("#canUseIntegral").val()==1){
						...
						$(".game_inner_btn_rest_inner svg use").attr("xlink:href", "#icon-aixin");
						$(".game_success_full").css("display", "block");
						$(".game_success_empty").css("display", "none");
					}
					if(gamePlayCost.result == 1){
						// 全局的 accountLogId
						accountLogId = gamePlayCost.accountLogId;
						//遮罩层
						$(".game_layer_cover").css("display", "none");
						//提示框
						$(".game_layer_success").css("display", "none");
					...
					if(gamePlayCost.result == 0){ //游戏代币不足
						// alert(111)
	...
	var integralPlayProcess = function(){
		...
		$.ajax({
			type : "POST",
			url : baseUrl + "puzzle-game-play-costintegral",
			data : {
			...
			success : function(msg) {
				if (msg.result.res == "ok") {
					var info = msg.result.gamePlayCost;
					$("span[name='accountval']").text(info.accountValue);
					if(info.accountValue == 0 && $("#canUseIntegral").val()==1){
						$(".game_inner_btn_rest_inner svg use").attr("xlink:href", "#icon-huiseaixin");
						$(".game_success_full").css("display", "none");
						$(".game_success_empty").css("display", "block");
					}else{
					...
					$("#times").html("0.00");
					if(info.result == 1){
						accountLogId = info.accountLogId;
						gameProcess();
						$(".game_btn_start_span").html("提示");
						$(".startGameStartButtonImgUrl").hide();
						$(".game_time").show();
	...
	var gameProcess = function(){
		...
		if($(".game_btn_start_span").html() == "开始") {
			$(".game_btn_start_span").html("提示");
			$(".startGameStartButtonImgUrl").hide();
			$(".game_time").show();
			$(".pic").css("opacity","1");
	...
	for(var i = 0; i < pic.length; i++) {
		...
		pic[i].addEventListener('touchmove', function(e) {
            ...
            if (newLeft >= picWidth*2) { newLeft = picWidth*2 }
            if (newtop >= picHeight*3) { newtop = picHeight*3 }
		...
		pic[i].addEventListener('touchend', function() {
			if(isSuccess() && boll) {
				...
				var url = baseUrl + "puzzle-game-play-record";
				var data = {
					activityId : activityId,
					type : opType,
					gameScore : seed,
				...
				$.ajax({
					...
					success : function(msg) {
						if (msg.result.res == "ok") {	
							var result = msg.result.gamePlayCost;		
							if(result.result == 1){					
								$("#gamescore").text((result.gameScore/100)+"秒");
								$("#gamescorerate").text((((1-(result.rank/result.allPlayerCount))*100).toFixed(2)) + "%");
								$("#bestgamescore").text((result.bestScore/100)+"秒");
								$("#bestrank").text("NO:" + result.bestRank);
								//$(".game_layer_rank_list_title p").html("当前名次对应：<span>一等奖</span>&nbsp;&nbsp;iPhone7一台");
								//遮罩层
								$(".game_layer_cover").css("display", "block");
								//提示框
								$(".game_layer_success").css("display", "block");
								$(".game_btn_start").show();
							}
						}						
						boll = false;
